...
### Script to perform Random Forest Classification (RF) on the trend component of all the feature between mouse that died (July 2019) and survived (October 2019)
...
png('RF_feat_imp_healthy_sick.png', width = 7, height = 10, units = "in", res = 300, pointsize = 12)
p <- imp %>% 
...
title <- ggdraw() + draw_label("Feature importance plot for Random Forest classification of \npolio infected mice health status (JUL, OCT 2019)", fontface='bold')
...
acc = c()
png('RF_ROC_healthy_sick.png', width = 6, height = 6, units = "in", res = 300, pointsize = 12)
...
for (i in 1:length(classes)) {
  ...
  if (i==1)
  {
    p1 <- plot(perf,main="ROC Curve for Random Forest classification of \npolio infected mice health status (JUL, OCT 2019)",
...
png('RF_CM_healthy_sick.png', width = 5.2, height = 5, units = "in", res = 300, pointsize = 12)
autoplot(cm, type = "heatmap", ) +
  scale_fill_gradient(low="#D6EAF8",high = "#2E86C1") +
  labs(x="True Class", y="Predicted Class", 
       title="Confusion Matrix for Random Forest classification of \npolio infected mice health status (JUL, OCT 2019)")
ttemp <- dev.off()
