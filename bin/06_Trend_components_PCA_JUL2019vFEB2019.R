...
### Script to perform principal component analysis (PCA) on the trend component of all the feature between mouse that infected and  died (July 2019) and that was uninfected (February 2019)
...
### Save the PCA plot
png('PCA_Uninfected_Sick_trend_all.png', width = 9, height = 7, units = "in", res = 300, pointsize = 12)
...
png('PCA_loadings_Uninfected_Sick_trend_all.png', width = 9, height = 7, units = "in", res = 300, pointsize = 12)
# Combine combined plot and legend using plot_grid()
p <- plot_grid(g2, legend, ncol=2, rel_widths = c(1, .35))
title <- ggdraw() + draw_label("PCA loadings plot on important trend features of \n polio infected and unifected mice that vary by measurement system (JUL, FEB 2019)", fontface='bold')
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1)) # rel_heights values control title margins
ttemp <- dev.off()
