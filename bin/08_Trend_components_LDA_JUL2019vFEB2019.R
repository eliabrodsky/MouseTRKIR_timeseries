...
### Script to perform linear discriminant analysis (LDA) on the trend component of all the feature between mouse that infected (July 2019) and uninfected (February 2019)
### input from the data folder (normalized data and decomposed data required)
### prints out to the LDA plot and LDA ordination plot to the figures folder
...
##### LDA for trend component
df_all <- MouseT1[c(23,6:21)]

### https://gist.github.com/thigm85/8424654
lda <- lda(formula = upd_is_sick ~ ., data = df_all)
prop.lda = lda$svd^2/sum(lda$svd^2)

plda <- predict(object = lda, newdata = df_all)
dataset = data.frame(upd_is_sick = df_all[,"upd_is_sick"],
                     lda = plda$x)


g1 <- ggplot(dataset) + geom_point(aes(lda.LD1, lda.LD2, colour = MouseT1$Days, shape = upd_is_sick), size = 2.5) + 
  labs(x = paste("LD1 (", percent(prop.lda[1]), ")", sep=""),
       y = paste("LD2 (", percent(prop.lda[2]), ")", sep="")) +
  scale_shape_discrete(labels = c("Healthy --> Died", "Healthy/Uninfected --> Survived", "Sick --> Died", "Sick --> Survived")) +
  scale_colour_viridis(limits = c(1,max(MouseT1$Days)), 
                       breaks = c(1:max(MouseT1$Days)),
                       labels = c(1:max(MouseT1$Days))) +
...
png('LDA_Uninfected_Sick_trend_all.png', width = 10, height = 7, units = "in", res = 300, pointsize = 12)
# Combine combined plot and legend using plot_grid()
p <- plot_grid(g1, legend, ncol=2, rel_widths = c(1, .35))
title <- ggdraw() + draw_label("LDA on decomposed trend features of \n polio infected mice vs uninfected mice (JUL, FEB 2019)", fontface='bold')
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1)) # rel_heights values control title margins
ttemp <- dev.off()


g2 <- ggord(lda, alpha=0, ylim=c(-6,15), xlim=c(-7,7)) + 
  geom_point(aes(dataset$lda.LD1, dataset$lda.LD2, colour = MouseT1$Days, shape = dataset$upd_is_sick), size = 2.5) + 
  labs(x = paste("LD1 (", percent(prop.lda[1]), ")", sep=""),
       y = paste("LD2 (", percent(prop.lda[2]), ")", sep="")) +
  scale_color_viridis(limits = c(1,max(MouseT1$Days)), 
                      breaks = c(1:max(MouseT1$Days)),
                      labels = c(1:max(MouseT1$Days))) +
  theme_bw(base_size = 14) +
  theme(legend.position = "none")
...
png('LDA_Ordn_Uninfected_Sick_trend_all.png', width = 9, height = 7, units = "in", res = 300, pointsize = 12)
# Combine combined plot and legend using plot_grid()
p <- plot_grid(g2, legend, ncol=2, rel_widths = c(1, .35))
title <- ggdraw() + draw_label("LDA Ordination plot on important trend features of \n polio infected mice vs uninfected (JUL, FEB 2019)", fontface='bold')
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1)) # rel_heights values control title margins
ttemp <- dev.off()
