### Data considered: uninfected mouse (8 weeks old) that survived (February 2019)
### Script to visualize the data smoothening and decomposition of time series data
### Script to automate STL decomposition for all the data from February 2019 measurements
...
# uninfected mouse (8 weeks old) that survived (February 2019 )

### Load one normalized data for visualization
M_feb2019_hlth <- read.csv(paste0(data_dir, "February2019/normalized data/top_left_healthy_1_ucsf.csv"))
...
time_diff <- (M_feb2019_hlth$time_mod[nrow(M_feb2019_hlth)] - M_feb2019_hlth$time_mod[1])
print(time_diff)

### Feature selection: temperature M_feb2019_hlth$temp
...
M_jul2019_temp_ts <- ts(M_feb2019_hlth$temp, frequency=as.numeric(time_diff)*60)

### convert the above time series to include time stamps
OFS_xts <- xts(M_jul2019_temp_ts, order.by = M_feb2019_hlth$time_mod)
...
temp_avg_3hr = c()
temp_avg_3hr_time = c()
for (i in 1:nrow(M_feb2019_hlth)){
  if (M_feb2019_hlth$time_mod[i] > (M_feb2019_hlth$time_mod[temp_start] + 60 * 60 * 3)) {
    temp_i <- i - 1
...
if(temp_start < nrow(M_feb2019_hlth)) { 
  temp_avg_3hr[temp_counter] <- mean(M_jul2019_temp_ts[temp_start:nrow(M_feb2019_hlth)])
  temp_avg_3hr_time = c(temp_avg_3hr_time,nrow(M_feb2019_hlth))
}

### convert the above averaged time series to include time stamps.
OFS_xts_avg_3hr <- xts(temp_avg_3hr, order.by = M_feb2019_hlth$time_mod[temp_avg_3hr_time])
...
### Feature selection: body speed M_feb2019_hlth$body_speed
### To decompose, we can create a time series by defining the frequency as the number of days times hours the samples are measured
M_jul2019_temp_ts <- ts(M_feb2019_hlth$body_speed, frequency=as.numeric(time_diff)*60)

### convert the above time series to include time stamps
OFS_xts_1 <- xts(M_jul2019_temp_ts, order.by = M_feb2019_hlth$time_mod)
...
### convert the above averaged time series to include time stamps.
OFS_xts_avg_3hr_1 <- xts(temp_avg_3hr, order.by = M_feb2019_hlth$time_mod[temp_avg_3hr_time])
...
png('data_smooth_uninfected.png', width = 8, height = 4, units = "in", res = 300, pointsize = 12)
# Combine combined plot and legend using plot_grid()
p <- plot_grid(combined_plot, legend,ncol=1,rel_heights = c(1, .12))
title <- ggdraw() + draw_label("Time series data for uninfected mice (FEB 2019)", fontface='bold')
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1)) # rel_heights values control title margins
ttemp <- dev.off()

###Plot for visualizing the decomposition of time series
png('data_decomp_uninfected.png', width = 10, height = 4, units = "in", res = 300, pointsize = 12)
p <- plot_grid(a1, a2, labels = "AUTO")
title <- ggdraw() + draw_label("Time series decomposition for uninfected mice (FEB 2019)", fontface='bold')
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1)) # rel_heights values control title margins
ttemp <- dev.off()


### automate the above process for all features in all the files in February2019/normalized data/ folder
lf <- list.files(paste0(data_dir, "February2019/normalized data"), pattern=("*.csv"))

for ( file.lst in lf) {
  print (paste0("Processing: ",data_dir, "July2019/normalized data/",file.lst))
  M_feb2019_hlth <- read.csv(paste0(data_dir, "February2019/normalized data/",file.lst))
  DT <- as.data.table(M_feb2019_hlth)
  M_feb2019_hlth <- as.data.frame(DT[, lapply(.SD,mean), by=c("date","time")])
  M_feb2019_hlth$date_mod <- anydate(M_feb2019_hlth$date)
  M_feb2019_hlth$time_mod <- as.POSIXct(paste(M_feb2019_hlth$date_mod, format(strptime(M_feb2019_hlth$time, "%I:%M:%S %p"), "%H:%M:%S")), format = "%Y-%m-%d %H:%M:%S")
  ...
  if(time_diff >=2) {
    M_feb2019_hlth_trend <- c()
    M_feb2019_hlth_seas <- c()
    for (i in 3:(ncol(M_feb2019_hlth)-2)){
      temp_ts <- ts(M_feb2019_hlth[,i], frequency=as.numeric(time_diff)*60)
      ...
      if(temp_start < nrow(M_feb2019_hlth)) { 
        temp_avg_3hr[temp_counter] <- mean(temp_ts[temp_start:nrow(M_feb2019_hlth)])
        temp_avg_3hr_time = c(temp_avg_3hr_time,nrow(M_feb2019_hlth))
      }
      ...
      M_feb2019_hlth_trend <- cbind(M_feb2019_hlth_trend, trendcycle(temp_ts_dec))
      M_feb2019_hlth_seas <- cbind(M_feb2019_hlth_seas, seasonal(temp_ts_dec))
    }
    
    colnames(M_feb2019_hlth_trend) <- paste0(colnames(M_feb2019_hlth[,c(-1,-2,-(ncol(M_feb2019_hlth)-1),-ncol(M_feb2019_hlth))]),"_tr")
    colnames(M_feb2019_hlth_seas) <- paste0(colnames(M_feb2019_hlth[,c(-1,-2,-(ncol(M_feb2019_hlth)-1),-ncol(M_feb2019_hlth))]),"_ss")
    
    write.table(M_feb2019_hlth_trend,file=paste0(data_dir, "February2019/decomposed data/",gsub(".csv","_tr.csv",file.lst)), sep=',',  quote = F, row.names = FALSE)
    write.table(M_feb2019_hlth_seas,file=paste0(data_dir, "February2019/decomposed data/",gsub(".csv","_ss.csv",file.lst)), sep=',',  quote = F, row.names = FALSE)
  } 
}
