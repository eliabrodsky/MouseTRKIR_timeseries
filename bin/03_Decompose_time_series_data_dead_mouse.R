### Data considered: Polio infected mouse (5 weeks old) that died (July 2019)
### Script to visualize the data smoothening and decomposition of time series data
### Script to automate STL decomposition for all the data from July 2019 measurements
...
# Polio infected mouse (5 weeks old) that died (July 2019)

### Load one normalized data for visualization
M_jul2019_sick <- read.csv(paste0(data_dir, "July2019/normalized data/avg_july_sick_1_ucsf.csv"))
...
time_diff <- (M_jul2019_sick$time_mod[nrow(M_jul2019_sick)] - M_jul2019_sick$time_mod[1])
print(time_diff)

### Feature selection: temperature M_jul2019_sick$temp
...
M_jul2019_temp_ts <- ts(M_jul2019_sick$temp, frequency=as.numeric(time_diff)*60)

### convert the above time series to include time stamps
OFS_xts <- xts(M_jul2019_temp_ts, order.by = M_jul2019_sick$time_mod)
...
temp_avg_3hr = c()
temp_avg_3hr_time = c()
for (i in 1:nrow(M_jul2019_sick)){
  if (M_jul2019_sick$time_mod[i] > (M_jul2019_sick$time_mod[temp_start] + 60 * 60 * 3)) {
    temp_i <- i - 1
...
if(temp_start < nrow(M_jul2019_sick)) { 
  temp_avg_3hr[temp_counter] <- mean(M_jul2019_temp_ts[temp_start:nrow(M_jul2019_sick)])
  temp_avg_3hr_time = c(temp_avg_3hr_time,nrow(M_jul2019_sick))
}

### convert the above averaged time series to include time stamps.
OFS_xts_avg_3hr <- xts(temp_avg_3hr, order.by = M_jul2019_sick$time_mod[temp_avg_3hr_time])
...
### Feature selection: body speed M_jul2019_sick$body_speed
### To decompose, we can create a time series by defining the frequency as the number of days times hours the samples are measured
M_jul2019_temp_ts <- ts(M_jul2019_sick$body_speed, frequency=as.numeric(time_diff)*60)

### convert the above time series to include time stamps
OFS_xts_1 <- xts(M_jul2019_temp_ts, order.by = M_jul2019_sick$time_mod)
...
### convert the above averaged time series to include time stamps.
OFS_xts_avg_3hr_1 <- xts(temp_avg_3hr, order.by = M_jul2019_sick$time_mod[temp_avg_3hr_time])
...
png('data_smooth_sick.png', width = 8, height = 4, units = "in", res = 300, pointsize = 12)
# Combine combined plot and legend using plot_grid()
p <- plot_grid(combined_plot, legend,ncol=1,rel_heights = c(1, .12))
title <- ggdraw() + draw_label("Time series data for polio infected mice that died (JUL 2019)", fontface='bold')
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1)) # rel_heights values control title margins
ttemp <- dev.off()

###Plot for visualizing the decomposition of time series
png('data_decomp_sick.png', width = 10, height = 4, units = "in", res = 300, pointsize = 12)
p <- plot_grid(a1, a2, labels = "AUTO")
title <- ggdraw() + draw_label("Time series decomposition for polio infected mice that died (JUL 2019)", fontface='bold')
plot_grid(title, p, ncol=1, rel_heights=c(0.1, 1)) # rel_heights values control title margins
ttemp <- dev.off()


### automate the above process for all features in all the files in July2019/normalized data/ folder
lf <- list.files(paste0(data_dir, "July2019/normalized data/"), pattern=("*.csv"))

for ( file.lst in lf) {
  print (paste0("Processing: ",data_dir, "July2019/normalized data/",file.lst))
  M_jul2019_sick <- read.csv(paste0(data_dir, "July2019/normalized data/",file.lst))
  M_jul2019_sick$date_mod <- anydate(M_jul2019_sick$date)
  M_jul2019_sick$time_mod <- as.POSIXct(paste(M_jul2019_sick$date_mod, format(strptime(M_jul2019_sick$time, "%I:%M:%S %p"), "%H:%M:%S")), format = "%Y-%m-%d %H:%M:%S")
  ...
  if(time_diff >=2) {
    M_jul2019_sick_trend <- c()
    M_jul2019_sick_seas <- c()
    for (i in 3:(ncol(M_jul2019_sick)-2)){
      temp_ts <- ts(M_jul2019_sick[,i], frequency=as.numeric(time_diff)*60)
      ...
      if(temp_start < nrow(M_jul2019_sick)) { 
        temp_avg_3hr[temp_counter] <- mean(temp_ts[temp_start:nrow(M_jul2019_sick)])
        temp_avg_3hr_time = c(temp_avg_3hr_time,nrow(M_jul2019_sick))
      }
      ...
      M_jul2019_sick_trend <- cbind(M_jul2019_sick_trend, trendcycle(temp_ts_dec))
      M_jul2019_sick_seas <- cbind(M_jul2019_sick_seas, seasonal(temp_ts_dec))
    }
    
    colnames(M_jul2019_sick_trend) <- paste0(colnames(M_jul2019_sick[,c(-1,-2,-(ncol(M_jul2019_sick)-1),-ncol(M_jul2019_sick))]),"_tr")
    colnames(M_jul2019_sick_seas) <- paste0(colnames(M_jul2019_sick[,c(-1,-2,-(ncol(M_jul2019_sick)-1),-ncol(M_jul2019_sick))]),"_ss")
    
    write.table(M_jul2019_sick_trend,file=paste0(data_dir, "July2019/decomposed data/",gsub(".csv","_tr.csv",file.lst)), sep=',',  quote = F, row.names = FALSE)
    write.table(M_jul2019_sick_seas,file=paste0(data_dir, "July2019/decomposed data/",gsub(".csv","_ss.csv",file.lst)), sep=',',  quote = F, row.names = FALSE)
  } 
}
